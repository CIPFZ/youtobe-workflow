...
ApiServer::ApiServer(TaskManager& manager,
                     MergeWorker& worker,
...
int ApiServer::start(unsigned short port) const {
    ...
    WFHttpServer server([this](WFHttpTask* task) {
        ...
        if (method == "POST" && uri == "/api/v1/subtitle/ass-to-mp4") {
            const void* body_ptr = nullptr;
            size_t body_len = 0;
            req->get_parsed_body(&body_ptr, &body_len);
            const std::string body(static_cast<const char*>(body_ptr), body_len);
            ...
            if (video_path.empty() || subtitle_path.empty() || output_path.empty()) {
                resp->set_status_code("400");
                resp->append_output_body("{\"error\":\"video_path/subtitle_path/output_path are required\"}");
                return;
            }

            bool reused = false;
            const std::string fp = build_subtitle_embed_fingerprint(video_path, subtitle_path, output_path);
            const std::string task_id = manager_.create_or_reuse(fp, video_path, subtitle_path, output_path, &reused);

            if (!reused) {
                manager_.update_status(task_id, TaskStatus::Running, 0, "subtitle embed worker started");
                std::thread([this, task_id, video_path, subtitle_path, output_path]() {
                    const int rc = subtitle_embed_worker_.run_ass_to_mp4(video_path, subtitle_path, output_path,
                        [this, task_id](int p, const std::string& msg) {
                            manager_.update_status(task_id, p >= 100 ? TaskStatus::Success : TaskStatus::Running, p, msg);
                        });
                    if (rc != 0) {
                        std::string err = "subtitle embed failed";
                        const auto rec = manager_.get_task(task_id);
                        if (rec.has_value() && !rec->message.empty()) {
                            err = rec->message;
                        }
